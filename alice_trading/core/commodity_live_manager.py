...
class CommodityLiveManager:
    """
    Multi-Tenant manager for live commodity futures market data.
    ...
    def __new__(cls, user_id="default"):
        if user_id not in cls._instances:
            instance = super(CommodityLiveManager, cls).__new__(cls)
    ...
    def set_credentials(self, user_id: str, api_key: str, totp_secret: str):
        ...
        self._totp_secret = str(totp_secret).strip()
    ...
    async def start(self):
        ...
        try:
            ...
            print(f"{LOG_PREFIX} [OK] Commodity live feed CONNECTED. "
                  f"Tracking: {list(self.resolved_instruments.keys())}")

        except Exception as e:
            print(f"{LOG_PREFIX} [ERROR] Start failed: {e}")
            traceback.print_exc()
            with self._lock:
    ...
    def _authenticate(self) -> bool:
        ...
        try:
            ...
            print(f"{LOG_PREFIX} [OK] Authentication successful.")
            return True
    ...
    def _resolve_contracts(self):
        ...
        """
        print(f"{LOG_PREFIX} [LOOKUP] Resolving MCX commodity contracts...")
        ...
        for base_sym in COMMODITY_SYMBOLS:
            ...
            if not resolved:
                print(f"{LOG_PREFIX} [ERROR] Could not resolve {base_sym}")
    ...
    def _resolve_from_csv(self, base_sym: str, mcx_df) -> bool:
        ...
        try:
            ...
            if inst:
                ...
                print(f"{LOG_PREFIX} [OK] {base_sym} resolved via CSV "
    ...
    def _resolve_generic(self, base_sym: str) -> bool:
        ...
        try:
            ...
            if inst:
                ...
                print(f"{LOG_PREFIX} [OK] {base_sym} resolved via generic API (token={inst.token})")
                return True
        except Exception:
            pass
        return False
    ...
    def _resolve_hardcoded(self, base_sym: str) -> bool:
        ...
        try:
            ...
            if inst:
                ...
                print(f"{LOG_PREFIX} [WARN] {base_sym} resolved via hardcoded token ({token})")
                return True
        except Exception:
            pass
        return False
                ...
                for sym, inst in self.resolved_instruments.items():
                    try:
                        try:
                            ...
                            self._alice.subscribe(inst)
                        print(f"{LOG_PREFIX} [WS] WS subscribed: {sym}")
                    except Exception as e:
    ...
    def _on_ws_open(self):
        print(f"{LOG_PREFIX} [WS] WebSocket CONNECTED")
    ...
    def _start_rest_pollers(self):
        ...
        for sym, inst in self.resolved_instruments.items():
            ...
            print(f"{LOG_PREFIX} [REST] REST poller started: {sym}")
    ...
    def _handle_disconnect(self):
        ...
        print(f"{LOG_PREFIX} [WARN] Connection lost. Initiating reconnect...")
        self._schedule_reconnect()
    ...
    async def _reconnect_loop(self):
        ...
        while self._running or self.status == "RECONNECTING":
            ...
            print(f"{LOG_PREFIX} [RECONNECT] Reconnect attempt in {wait_time}s...")
            ...
            try:
                if self._authenticate():
                    ...
                    if self.resolved_instruments:
                        ...
                        print(f"{LOG_PREFIX} [OK] Reconnected successfully!")
                        return
            except Exception as e:
