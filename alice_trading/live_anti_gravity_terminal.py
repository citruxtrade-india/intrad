...
INSTRUMENTS_CONFIG = {
    ...
    "HINDUNILVR":  {"exchange": "NSE", "token": 1394, "name": "HINDUNILVR"},
    "HDFCBANK":    {"exchange": "NSE", "token": 1333, "name": "HDFCBANK"}
}

DISPLAY_ORDER = ["NIFTY 50", "BANK NIFTY", "SENSEX", "GOLD MCX", "SILVER MCX", "CRUDEOIL", "NATGAS", "HINDUNILVR", "HDFCBANK"]

# ---------------- STATE ---------------- #
state = {name: {"price": 0.0, "vol": 0.0, "src": "INIT", "ts": "--:--:--"} for name in DISPLAY_ORDER}
token_to_name = {} # token_str: display_name
last_ws_tick = {name: 0.0 for name in DISPLAY_ORDER}
lock = threading.Lock()
...
def refresh_ui():
    ...
    for name in DISPLAY_ORDER:
        ...
        p_str = f"{p_val:<10.2f}"
        vol_val = float(s['vol'])
        v_str = f"{vol_val:.0f}" if vol_val > 0 else "0.0"
        line = f"MKT {s['ts']} | {name:<10}: {p_str} | Vol: {v_str:<8} | Src: {s['src']}"
...
if __name__ == "__main__":
    ...
    def on_open():
        try:
            ...
            alice.subscribe(ws_subs, LiveFeedType.MARKET_DATA)
        except:
