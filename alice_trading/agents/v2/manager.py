...
class AgentEvent:
    ...
    def to_dict(self):
        # Determine normalized signal
        signal = "NONE"
        if self.state == "APPROVED":
            if "BUY" in self.reason.upper() or "LONG" in self.reason.upper(): signal = "BUY"
            elif "SELL" in self.reason.upper() or "SHORT" in self.reason.upper(): signal = "SELL"
        
        return {
            "agent": self.agent,
            "symbol": self.symbol,
            "signal": self.payload.get("signal", signal),
